...
> ✅ **Todos os 5 itens abaixo foram concluídos** (confirmado em 27/08/2026, ao resgatar este
> arquivo — que só existia no servidor de produção, nunca tinha sido versionado no git — durante
> uma investigação de tooling de deploy). Prompts mantidos abaixo como registro histórico do que
> foi pedido originalmente; ver `CLAUDE.md` e `memoria-tecnica/` pro estado atual de cada item.
>
> | Item | Status |
> |---|---|
> | AUD-01 — Rotação AES-128→AES-256 | ✅ Concluído (`EncryptionConverter`, `EncryptionMigrationRunner`; migração final rodada em produção em 27/08/2026, 1026 registros) |
> | AUD-04 — Segredos em Docker Secrets | ✅ Concluído (`docker-compose.yml` + `spring.config.import=optional:configtree:/run/secrets/`) |
> | SEC-01 — JWT em cookie HttpOnly | ✅ Concluído (`AuthController`, cookie `token` HttpOnly+Secure+SameSite=Strict) |
> | SEC-02 — MFA/TOTP completo | ✅ Concluído em 27/08/2026 (`MfaController`/`MfaService`/`TotpService`) — implementado pra ADMIN, PROFESSIONAL **e** PATIENT (mais amplo que o pedido original, que era só ADMIN), com códigos de backup (não pedidos originalmente, adicionados por decisão consciente) |
> | SEC-03 — Refresh token rotativo + denylist | ✅ Concluído (`RefreshToken`/`RefreshTokenService`/`TokenDenylistService`) |
